...
/* Read */// WLM--------------------------
    class_call(parser_read_list_of_integers(pfc,"selection_list",&(int1),&(pointer2),&flag1,errmsg),
               errmsg,
               errmsg);
    /* Complete set of parameters */
if (flag1 == _TRUE_){   
 for(i=0; i<int1; i++) {
if (pointer2[i]<=3) {
ppt->selection_list[i]=pointer2[i];
//printf("%d",ppt->selection_list[i]);
}
if (pointer2[i] == gaussian) {
printf("Bin %d: gaussian\n",i);
}
else if (pointer2[i] == tophat) {
printf("Bin %d: tophat\n",i);
}
else if (pointer2[i] == dirac) {
printf("Bin %d: dirac\n",i);
}
else if (pointer2[i] == ffunc) {
printf("Bin %d: ffunc\n",i);
}
else if (pointer2[i] == zffunc) {
printf("Bin %d: zffunc\n",i);
}
else {
class_stop(errmsg,"In selection function input: type %s is unclear",pointer2[i]);
}
}
free(pointer2);
} 
//-----------------------------------
    ...
    if ((flag1 == _TRUE_) && (int1>0)) {
      ...
      for (i=1; i<int1; i++) {       // first set all widths to default; correct eventually later
        ...
        ptr->selection_bias2[i] = ptr->selection_bias2[0];
        ptr->selection_bias_type[i] = ptr->selection_bias_type[0];// WLM
...
// WLM
      ...
      /* Read */
      class_call(parser_read_list_of_doubles(pfc,"selection_bias2",&int1,&pointer1,&flag1,errmsg),
                 errmsg,
                 errmsg);
      /* Complete set of parameters */
      if ((flag1 == _TRUE_) && (int1>0)) {
        if (int1==1) {
          for (i=0; i<ppt->selection_num; i++) {
            ptr->selection_bias2[i] = pointer1[0];
          }
        }
        else if (int1==ppt->selection_num) {
          for (i=0; i<int1; i++) {
            ptr->selection_bias2[i] = pointer1[i];
          }
        }
        else {
          class_stop(errmsg,
      ...
      /* Read */
          class_call(parser_read_list_of_integers(pfc,"selection_bias_type",&(int1),&(pointer2),&flag1,errmsg),
                 errmsg,
                 errmsg);
      /* Complete set of parameters */
      if ((flag1 == _TRUE_) && (int1>0)) {
        if (int1==1) {
          for (i=0; i<ppt->selection_num; i++) {
            ptr->selection_bias_type[i] = pointer2[0];
          }
        }
        else if (int1==ppt->selection_num) {
          for (i=0; i<int1; i++) {
            ptr->selection_bias_type[i] = pointer2[i];
          }
        }
        else {
          class_stop(errmsg,
                     "In input for selection function, you asked for %d bin centers and %d bin biase types; number of bins unclear; you should pass either one bin bias type (common to all bins) or %d bin biase types.",
                     ppt->selection_num,int1,ppt->selection_num);
        }
        free(pointer2);
      }
...
int input_default_params(struct background *pba,
  ...
  /** 2.a) Selection functions W(z) of each redshift bin */
  ppt->selection=gaussian;
  ppt->selection_list[0]=0;
  ppt->selection_num=1;
  ppt->selection_mean[0]=1.;
  ppt->selection_width[0]=0.1;
  ptr->selection_bias[0]=1.;
  ptr->selection_bias2[0]=0.;
  ptr->selection_bias_type[0]=0;// WLM
